...
fun OtpScreen(otp:String,flashViewModel: FlashViewModel,callbacks:PhoneAuthProvider.OnVerificationStateChangedCallbacks) {
    ...
    Button(onClick = {
        if(otp.isEmpty()){
            ...
            signInWithPhoneAuthCredential(credential=credential,context=context,flashViewModel=flashViewModel)
        }
    ...
    , color = Color(63,81,181,255),
        ...
        modifier = Modifier.clickable {
                ...
                val options = PhoneAuthOptions.newBuilder(auth)
                    ...
                    .setTimeout(60L, TimeUnit.SECONDS) // 60 seconds ke baad hi new otp request kr payunga. usse phle mahi
...
BasicTextField(value = otp, onValueChange = {flashViewModel.setOtp(it)}, modifier = Modifier.fillMaxWidth(), singleLine = true){
    Row(horizontalArrangement = Arrangement.Center) {
        repeat(6){index->//iteration parameter
            val number=when{
                index >= otp.length -> ""
